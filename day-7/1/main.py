...
def get_input():
    global input_idx, input_buf
    global pc, mem
    input_idx += 1
    return input_buf[input_idx-1]
    #return int(input("{:>05d}< ".format(pc)))

output_buf = []

def output(out):
    global pc, mem, output_buf
...
def run():
    global pc, mem
    global input_idx, output_idx, output_buf
    pc = 0
    input_idx = 0
    output_idx = 0
    output_buf = []
...
settings = list(itertools.permutations(range(5)))
