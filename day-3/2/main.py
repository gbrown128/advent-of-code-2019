...
Each line entry will have:
    ...
    + The total "steps" taken to the line's start

Perpendicular line segments between data sets will be tested and
crossings identified. We add the distances to the crossings, and store
the total timing.
"""
...
for instruction in wire1:
    ...
    w1d.append([ax, pos[1-ax], start, end, steps])
...
for instruction in wire2:
    ...
    w2d.append([ax, pos[1-ax], start,  end, steps])
    pos[ax] = end
    steps += length
...
for seg1 in w1d:
    for seg2 in w2d:
        ...
        if not seg1[0] == seg2[0]:
            if seg2[1] in range(min(seg1[2], seg1[3]), max(seg1[2], seg1[3]))\
               and seg1[1] in range(min(seg2[2], seg2[3]), max(seg2[2], seg2[3])):
                # Intercept found. Now we need the steps to it.
                w1steps = abs(seg1[2]-seg2[1]) + seg1[4]
                w2steps = abs(seg2[2]-seg1[1]) + seg2[4]
                intercepts.append(w1steps+w2steps)
